...
const SUB_PATH       = process.env.SUB_PATH       || 'sub999';      // 订阅sub路径，默认为sub
const UUID           = process.env.UUID           || '78472f44-65cb-4acb-aea0-477a68aef64a'; // UUID，运行哪吒请修改
const NEZHA_SERVER   = process.env.NEZHA_SERVER   || 'nazha.xccssaa.dpdns.org:443';         // 哪吒面板地址，v1形式：nz.serv00.net:8008
const NEZHA_PORT     = process.env.NEZHA_PORT     || '';         // v1哪吒请留空，v0 agent端口
const NEZHA_KEY      = process.env.NEZHA_KEY      || 'EsGSPZ2kMIui9yfuvmQuzS7YSuIf6yrh';         // v1的NZ_CLIENT_SECRET或v0 agent密钥
const ARGO_DOMAIN    = process.env.ARGO_DOMAIN    || 'vercel.xchwwss.dpdns.org';         // argo固定隧道域名,留空即使用临时隧道
const ARGO_AUTH      = process.env.ARGO_AUTH      || 'eyJhIjoiMjBhN2JkMTA3ZTZhNzk1YTM3Zjk1ZTYxZTYyMTA3MDkiLCJ0IjoiM2ZhMDE2Y2ItY2RiMy00YmM4LWJkMGMtODRkYTBmZmRmYzY1IiwicyI6IllXSTNPV0l6TVRFdE1HTTBOUzAwWkRCbUxXSXhabUV0Wm1NeVptVmhZV0kyWVdFMyJ9';         // argo固定隧道token或json,留空即使用临时隧道
...
const CHAT_ID        = process.env.CHAT_ID        || '7886284400';         // Telegram chat_id，两个变量不全不推送
const BOT_TOKEN      = process.env.BOT_TOKEN      || '7669258945:AAGNTd8625Oy6h3oWN8en1EfDn2ZY0BjpHc';         // Telegram bot_token，两个变量不全不推送
